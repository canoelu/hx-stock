...
def stock_trade_date():
    calendar_data = ak.tool_trade_date_hist_sina()
    trade_date=set(calendar_data['trade_date'].values.tolist())
    return trade_date
...
def get_trade_hist_interval(date):
    """
    获取指定日期之前三年的日期范围，并检查指定日期是否为交易日的开盘到收盘之间。

    Parameters:
    - date: 指定日期，格式为 "YYYY-MM-DD"

    Returns:
    - date_start: 三年前的日期，格式为 "YYYYMMDD"
    - is_trade_date_open_close_between: 指定日期是否为交易日的开盘到收盘之间，True 或 False
    """
...
if __name__ == '__main__':
    res=get_previous_trade_date('2024-04-28')
    print(res)
